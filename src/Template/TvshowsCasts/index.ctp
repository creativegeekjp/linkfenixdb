...
<div class="tvshowsCasts index large-9 medium-8 columns content">
    <h3><?= __('Tvshows Casts') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th><?= $this->Paginator->sort('id') ?></th>
                <th><?= $this->Paginator->sort('tvshow_id') ?></th>
                <th><?= $this->Paginator->sort('cast_id') ?></th>
                <th class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($tvshowsCasts as $tvshowsCast): ?>
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $tvshowsCast->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $tvshowsCast->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $tvshowsCast->id], ['confirm' => __('Are you sure you want to delete # {0}?', $tvshowsCast->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
